...
router.patch("/me/profile", requireAuth, banCheck, async (req: Request, res: Response) => {
  const { userId } = getAuth(req);

  const { bgmiUid, bgmiIgn, bgmiTier, bio } = req.body as {
    bgmiUid?: string;
    bgmiIgn?: string;
    bgmiTier?: BgmiTier;
    bio?: string;
